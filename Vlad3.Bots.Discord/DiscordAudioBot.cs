...
public sealed class DiscordAudioBot : IAudioBot
{
    ...
    public Task ConnectAsync(string channelId, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Discord bot {Label} connected to channel {Channel}", Label, channelId);
    ...
    public Task DisconnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Discord bot {Label} disconnected", Label);
        return Task.CompletedTask;
    }
    ...
    public Task PlayAsync(AudioTrackInfo track, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Discord bot {Label} playing {Track}", Label, track.Name);
        return Task.CompletedTask;
    }
    ...
    public Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Discord bot {Label} stopped playback", Label);
        return Task.CompletedTask;
    }

    public Task RegisterCommandsAsync(CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Discord bot {Label} registered commands", Label);
        return Task.CompletedTask;
    }

    public ValueTask DisposeAsync()
    {
        _logger.LogInformation("Discord bot {Label} disposed", Label);
    ...
    private static List<AudioChannelInfo> BuildChannels(BotConfiguration configuration)
    {
            ...
            && !string.IsNullOrWhiteSpace(rawChannels))
        {
            var channels = rawChannels
                ...
                .Select((name, index) => new AudioChannelInfo($"discord-{index + 1}", name))
                .ToList();
        ...
        {
            new("discord-1", "Discord General"),
            new("discord-2", "Discord Music")
        };
    }
}
